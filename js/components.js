...
// data = a function returning the content (actually HTML)
	...
	Vue.component('tree', {
		...
		template: `<div>
		<span class="upgRow" v-for="(row, r) in data" style='margin-top: -4px;'>
		<table style='width:100%'>
			<span v-for="(node, id) in row" style = 'width:100%;display:block;overflow: hidden;transition-duration:1s' :style='maxHeight(node)'>
			<tree-node :layer='node' :prev='layer' :abb='tmp[node].symbol' :key="key + '-' + r + '-' + id"></tree-node>
			<tab-buttons v-bind:style="tmp[node].componentStyles['tab-buttons']" :layer="node" :data="tmp[node].tabFormat" :name="'mainTabs'"></tab-buttons>
			</span>
		</span></div>
