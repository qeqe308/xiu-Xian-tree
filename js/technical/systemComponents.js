var systemComponents = {
	'tab-buttons': {
		...
		template: `
			<div class="upgRow">
				<div v-for="tab in Object.keys(data)" style='width:100%'>
					<button 
					v-if="data[tab].unlocked==undefined|| data[tab].unlocked" 
					v-bind:class="{
						accc:player.tab==layer&&player.subtabs[layer][name]==tab,
						kuanduyibai:true,tabButton: true, notify: subtabShouldNotify(layer, name, tab), resetNotify: subtabResetNotify(layer, name, tab)}"
					v-bind:style="[
						{'background-color': tmp[layer].color},
						 (data[tab].glowColor && subtabShouldNotify(layer, name, tab) ? {'box-shadow': 'var(--hqProperty2a), 0 0 20px '  + data[tab].glowColor} : {}),
						  tmp[layer].componentStyles['tab-button'],
						   data[tab].buttonStyle]"
						v-on:click="function(){player.tab=layer;player.subtabs[layer][name] = tab; updateTabFormats(); needCanvasUpdate = true;}">{{tab}}</button>
				</div>
			</div>
		`
	},
	...
	'tree-node': {
		...
		<button v-if="nodeShown(layer)"
			v-bind:id="layer"
			v-on:click="function() {
				if(layers[layer].tabFormat&&!Array.isArray(tmp[layer].tabFormat)){
					player.suptabs[layer]?player.suptabs[layer]=null:player.suptabs[layer]=1
				}
				else {
			...
			<span v-if='layers[layer].tabFormat&&!Array.isArray(tmp[layer].tabFormat)&&player.qitaxinxi.jiantou==1' class='jiantou' :style='rotate(layer,"右")'>→</span>
			<span v-if='layers[layer].tabFormat&&!Array.isArray(tmp[layer].tabFormat)&&player.qitaxinxi.jiantou==0' v-bind:class='{zhankai:player.suptabs[layer]==1,jieshu:!player.suptabs[layer]==1}' style='transition-duration:1s'>    > > ></span>
	...
	'layer-tab': {
		...
		<div v-if="tmp[layer].tabFormat">
			...
			<div v-else>
				<div v-if='false' class="upgTable" v-bind:style="{'padding-top': (embedded ? '0' : '25px'), 'margin-top': (embedded ? '-10px' : '0'), 'margin-bottom': '24px'}">
